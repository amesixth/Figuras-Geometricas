...
public class LinDiagonal extends Canvas {

    int ejeX, ejeY;
    float gx = 0, gy = 0;
    int minX, minY, maxX, maxY;
    JTextField[] tf = InterfazLinDia.tf;
    DefaultTableModel dtm = InterfazLinDia.dtm;
    JComboBox cb = InterfazLinDia.colores;
    boolean y = InterfazLinDia.y;
    ...
    public void paint(Graphics g) {
        ...
        for (int i = minX, k = minY, j = 1; i <= maxX || k >= maxY; i += 10, k -= 10, j++) {
            g.fillOval(i, k, 6, 6);
            String[] s = {"P" + j, gx + "", gy + ""};
            dtm.addRow(s);
            gx += 0.5;
        ...
        g.drawString("Pi ("+mX+", "+mY+")", minX+15, minY);
        g.drawLine(minX + 2, minY+2, maxX + 2, maxY+2);
